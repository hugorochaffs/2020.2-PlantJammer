...
| ITP53 | O aplicativo deve enviar os [logs de execução](pages/ponto_de_controle_3/lexico?id=Logs-de-execução) para um servidor, para futura análise de bugs | Requisito Não Funcional |
...
| ITP56 | O aplicativo não deve consumir realizar downloads quando estiver do [background](pages/ponto_de_controle_3/lexico?id=Background) | Requisito Não Funcional |
