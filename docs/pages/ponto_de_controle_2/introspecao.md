...
A introspeção é o primeiro e o mais óbvio método para tentar entender quais as funcionalidades e restrições que o sistema deve está sujeito para que o projeto tenha sucesso.

A técnica de introspecção consistem em um método reflexivo de elicitação de requisitos muito rico e profundo, e de baixo custo. Trata-se de entender quais as prioridades que o sistema deve possuir para que seja bem recebido pelos clientes e pelos stakeholders. Nessa técnica o engenherio de requisitos deve imaginar o que os usuários e stakeholders gostariam de ver presente no sistema.

Por se basear na intuição do engenheiro de requisitos, nem sempre a técnica de introspecção obtem requisitos verdadeiros, uma vez que o engenheiro possui uma visão enviesada e não é capaz de ter a visão de todos os tipos de usuários que estaram envolvidos com o sistema.

## 2. Objetivo

O objetivo dessa técnica é fazer que o analista de requisitos imagine o que os potenciais usuários do sistema desejam no sistema, e assim levantar requisitos que serão analisados e validados em etapas posteriores do projeto.

## 3. Introspecção

| Identificador (ID) | Descrição | Prioridade (MosCoW) | Persona | Tipo |
| :----------------: | :-------: | :--------: | :-----: | :--: |
|  | Buscar uma receita pelo nome (pizza de..., sopa de..., etc) | Must | Gabriela Rocha da Silva | Requisito Funcional |
|  | Compartilhar o link para um receita |  | Would | Requisito Funcional |
|  | O layout do aplicativo deve ser responsivo para os variados tamanho de telas | Must | Gabriela Rocha da Silva | Requisito Não Funcional |
|  | Ter a opção de fazer login através do Facebook, Google | Would | Gabriela Rocha da Silva | Requisito Funcional |
|  | Ter a opção de fazer login usando email/username e senha | Must | Gabriela Rocha da Silva | Requisito Funcional |
|  | Ter a opção de realizar cadastro através do Facebook, Google | Would | Gabriela Rocha da Silva | Requisito Funcional |
|  | Ter a opção de realizar cadastro usando email/username e senha | Must | Gabriela Rocha da Silva | Requisito Funcional |
...
|  | Ter a opção de recuperar a senha esquecida | Must | Gabriela Rocha da Silva | Requisito Funcional |
|  | Durante o cadastro o usuário deve ser apresentado aos Termos e Políticas do APP | Must |  | Requisito Funcional |
|  | Ter a opção de alterar o nome do perfil cadastrado | Should | Gabriela Rocha da Silva | Requisito Funcional |
|  | Ter a opção de alterar o email do perfil cadastrado | Should | Gabriela Rocha da Silva | Requisito Funcional |
|  | Ter a opção de alterar a senha do perfil cadastrado | Must | Gabriela Rocha da Silva | Requisito Funcional |
|  | O usuário deve ser capaz de utilizar o aplicativo mesmo sem estar logado | Must | Gabriela Rocha da Silva | Requisito Funcional |
|  | O aplicativo deve ter a opção de deletar a conta  | Must | Gabriela Rocha da Silva | Requisito Funcional  |
|  | Os dados dos usuários devem ser apagados ou anonimados após a conta ser deletada (LGPD)  | Must | Gabriela Rocha da Silva | Requisito Funcional |
|  | O usuário deve poder utilizar a versão Premium do aplicativo durante alguns dias | Should | Gabriela Rocha da Silva | Requisito Funcional |
...
|  | Na tela da receita deve ser listado os ingredientes e as respectivas quantidades necessárias para se produzir a receita | Must | Gabriela Rocha da Silva | Requisito Funcional |
